...
- 支持 JPEG、PNG、GIF、TIFF、BMP、HEIC、HEIF、WebP、AVIF、SVG，以及 Sony ARW 和 Nikon NEF（RAW；只读）。
- 同名 ARW/NEF 与 JPG 自动组合显示；移到废纸篓、撤销和重做会将整个组合视为一个操作。
...
- 旋转、镜像、自由裁剪、20 步撤销/重做、保存、另存为和未保存编辑确认；RAW 原始文件保持只读。
...
- 可从应用菜单检查 GitHub Release 更新，发现新版本后直接打开对应 DMG 下载地址。
